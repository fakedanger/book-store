            ...
            if forms.is_valid():
                ...
                for s in cart.objects.filter(Q(name=request.user.first_name)&Q(Returndate__isnull=True)&Q(orderno=ordernoquery)&Q(fkbook__isbn=isbnquery)):
                    if s.fkbook.Category == 'Ebook':
                        ...
                        send_mail('Order Confirmation',notification, settings.EMAIL_HOST_USER,['vy363394@gmail.com'],fail_silently=False)
...
def verifyfun(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject='UMANG NGO'
            msg='THANKYOU TO SUBSCRIBING EMAIL ..!!'
            send_mail(subject,msg,'vy363394@gmail.com',[cd['email']])
            form.save()
			#em=form.cleaned_data
            #try:
                #for s in verify.objects.filter(Q(email=em)&Q(otp__isnull=True)):
            ...
            #send_mail('html_content',notification,'ajaykadyan92@jims.com',[em['email']])
    ...
    if request.method=='POST':
        ...
        if forms.is_valid():
            ...
            if request.user.first_name:
                blog.author = request.user.first_name
            else:
...
@login_required
def mypost(request):
    n=blog.objects.filter(author=request.user.first_name).order_by('-date_created')
    j=User.objects.get(Q(username=request.user) | Q(first_name=request.user.first_name))
...
def com(request,d):
    ...
    if request.method=="POST":
        ...
        if forms.is_valid():
            ...
            osc.bg=n
            osc.name=request.user.first_name
            forms.save()
            return HttpResponseRedirect('/home')
    else:
        forms=commentform()
...
def resetpassword(request,d):
    user=resetpass.objects.get(serialid=d)
    if request.method == "POST":
            ...
            try:
                ...
                for s in resetpass.objects.filter(Q(email=email)&Q(user=username)&Q(serialid__isnull=True)):
                    ...
                    if resetpass.objects.filter(Q(serialid=s.serialid)):
                        ...
                        s.save()
                    html_content="http://127.0.0.1:8000/resetpass/%s/"%(s.serialid)
...
def mycart(request):
    ...
    for i in cart.objects.filter(Q(name=request.user.first_name)&Q(Purchasedate=None)):
        if i.fkbook.stock<1:
            ...
            i.delete()
    n=book.objects.all()
    return render(request,'mycart.html',{'cart':prod,'n':n})
...
def invoice(request):
    ...
    if request.user:
        ...
        html_content="Your order no is %s \n\nShipping address: %s, %s, \n\nMobile No. %s\n\nInvoice: http://127.0.0.1:8000/%s\n\n Contact Us: vy363394@gmail.com"%(jks, j.address, j.pincode, j.mob,jks)
        notification="Name: %s %s,   Order no is %s     Shipping address: %s, %s,   Mobile No. %s"%(request.user.first_name, request.user.last_name,jks, j.address, j.pincode, j.mob)
        send_mail('Order Confirmation',html_content, settings.EMAIL_HOST_USER,[request.user.email],fail_silently=False)
        send_mail('Order Confirmation',notification, settings.EMAIL_HOST_USER,['vy363394@gmail.com'],fail_silently=False)
        return render(request,'invoice.html',{'n':n,'j':j})
